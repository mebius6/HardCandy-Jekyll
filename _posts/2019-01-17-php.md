...
- 与heredoc的写法差不多，唯一的区别是必须在结束标识符要用单引号括起来。
是类似于单引号字符串的
语法：<<<标识
字符串
标识；
...
- 参数1 :str 规定被搜索的字符串。
- 参数2 :search 规定要搜索的字符串。如果该参数是数字，则搜索匹配该数字对应的 ASCII 值的字符。
- 参数3:before_search 可选。一个默认值为 "false" 的布尔值。如果设置为 "true"，它将返回 search 参数第一次出现之前的字符串部分。
